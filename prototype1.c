...
// FALL DETECTION
bool detectFall(HCSR04_t *sensor1, HCSR04_t *sensor2,
                float prev_d1, float prev_d2) {
    float curr_d1 = distance(sensor1);
    float curr_d2 = distance(sensor2);
    ...
    // sudden distance change > 20cm = fall
    ...
    *hex = 0x00000000;
    ...
    while(1) {
        ...
        if(vibrationRead(&vib)) {
            ...
            if(detectFall(&sensor1, &sensor2, prev_d1, prev_d2)) {
                assist_mode = 1;
                show_assist();
        ...
        // --- SLOW FALL DETECTION ---
        // if distances not changing and cane idle for too long
        ...
        if(!assist_mode) {
            ...
            // show distance on HEX when not in assist mode
