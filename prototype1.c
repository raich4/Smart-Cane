...
int main(void) {
    ...
    // A "tap window" opens on the first vibration hit.
    // We accumulate taps for TAP_WINDOW loop iterations,
    // then evaluate the total count — no blocking, no missed taps.
    int tap_count         = 0;
    int tap_armed         = 0;   // 1 = window is open
    int tap_timer         = 0;
    int prev_vib          = 0;   // edge detection: only count rising edges
    #define TAP_WINDOW    200    // tune: more iterations = wider tap window
    // ────────────────────────────────────

    GPIO_t *jp2 = (GPIO_t *)JP2_BASE;

    // sensor 1: trig=bit5, echo=bit6
    HCSR04_t sensor1 = { jp2, 5, 6 };
    pinMode(sensor1.gpio, sensor1.trig_pin, 1);
    pinMode(sensor1.gpio, sensor1.echo_pin, 0);

    // sensor 2: trig=bit3, echo=bit4
    ...
    *hex54 = 0x00000000;
    ...
    while (1) {
        ...
        delay(10000);                   // reduced from 60000 — less dead time
        float dist2 = distance(&sensor2);
        delay(10000);

        // ── Vibration tap state machine ──────────────────────────────────
        // Uses rising-edge detection so a sustained HIGH only counts once.
        int vib_now = vibrationRead(&vib);
        int rising_edge = (vib_now && !prev_vib);
        prev_vib = vib_now;

        if (rising_edge) {
            if (!tap_armed) {
                // First tap: open the window
                tap_count = 1;
                tap_armed = 1;
                tap_timer = 0;
            } else {
                // Additional tap within the window
                tap_count++;
                tap_timer = 0;  // reset window on each new tap
            }
        }
        ...
        if (tap_armed) {
            ...
            if (tap_timer >= TAP_WINDOW) {
                // ── Window expired: evaluate tap count ───────────────
                if (detectFall(dist1, dist2, prev_d1, prev_d2)) {
                    // Sudden change in sensor readings → fall event
                    assist_mode = 1;
                    show_assist();
                    play_emergency_siren(0x3FFFFFFF);
                }
                else if (tap_count <= 2 && !assist_mode) {
                    // 1 or 2 taps → activate assist mode
                    assist_mode = 1;
                    show_assist();
                    play_emergency_siren(0x3FFFFFFF);
                }
                else if (tap_count >= 3 && assist_mode) {
                    // 3+ taps → disengage assist mode
                    assist_mode = 0;
                    show_disengage();
                    delay(1000000);
                    *hex   = 0x00000000;
